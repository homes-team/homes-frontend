...
function CategoryChips({ value, onChange }: CategoryChipsProps) {
  return (
    <div className="flex flex-wrap gap-2" role="group" aria-label="방 종류">
      {CATEGORIES.map((category) => {
        ...
        return (
          <button
            key={category.label}
            type="button"
            className={`rounded-pill border px-3.5 py-[7px] text-[13px] font-medium whitespace-nowrap transition-colors ${
              active
