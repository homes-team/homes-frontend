 ...
 * (checkEmailDuplicate/sendSignupEmailCode/verifySignupEmailCode는 두 가입 유형이
 * 동일한 Redis AUTH_SUCCESS 플래그를 공유하므로 그대로 재사용 가능하다.)
 */
export function useEmailAccountStep() {
  ...
  function handleEmailChange(value: string) {
    setEmail(value);
    // 이메일을 바꾸면 이전 중복확인/인증 결과는 더 이상 유효하지 않으므로 초기화
